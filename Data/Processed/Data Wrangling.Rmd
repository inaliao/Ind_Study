...
df_2012<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2012.csv'),skip=2,stringsAsFactors = TRUE)
df_2013<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2013.csv'),skip=2,stringsAsFactors = TRUE)
df_2014<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2014.csv'),skip=2,stringsAsFactors = TRUE)
df_2015<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2015.csv'),skip=2,stringsAsFactors = TRUE)
df_2016<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2016.csv'),skip=2,stringsAsFactors = TRUE)
df_2017<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2017.csv'),skip=2,stringsAsFactors = TRUE)
df_2018<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2018.csv'),skip=2,stringsAsFactors = TRUE)
df_2019<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2019.csv'),skip=2,stringsAsFactors = TRUE)
df_2020<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2020.csv'),skip=2,stringsAsFactors = TRUE)
df_2021<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2021.csv'),skip=2,stringsAsFactors = TRUE)
df_2022<-read.csv(here('Data/Raw/weather data/1044988_35.21_-80.86_2022.csv'),skip=2,stringsAsFactors = TRUE)

raw_weather<-bind_rows(df_2012,df_2013,df_2014,df_2015,df_2016,df_2017,df_2018,df_2019,df_2020,df_2021,df_2022)
raw_weather
```

```{r Selected needed data - load}
df_DUK<-raw_DUK[,c(5,8,9,10,11)]
head(df_DUK)
colnames(df_DUK)
...
df_DUK$Local.time<-as.POSIXct(df_DUK$Local.Time,format = "%d%b%Y %H:%M:%S")
df_DUK$Local.time<-format(df_DUK$Local.time,"%Y-%m-%d %H:%M")

df_DUK<-df_DUK[,c(1,2,8,3:6)]
...
df_DUK$day_of_week <- wday(df_DUK$Date, label = FALSE)
df_DUK<-df_DUK[,c(1:3,9,4:8)]
head(df_DUK)
```
...
```{r Export file}
write.csv(df, file = "Data/Processed/Load_weather.csv",row.names = TRUE)
```
