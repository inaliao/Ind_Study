...
#electricity 
plot_msts_electricity<-autoplot(msts_all[,6:10])+
  labs(y="electicity (MWh)")
plot_msts_electricity

#weather
plot_msts_weather<-autoplot(msts_all[,1:5])
plot_msts_weather 
#this plot does not have any meaning given that the unit of the variables are different
...
#decompose 
demand_decompose<-decompose(ts_all[,7],type="additive")
...
#plot
plot_grid(
  autoplot(Acf(ts_all[,7],lag=40,plot=FALSE),main="Original Electricity Demand"),
  autoplot(Pacf(ts_all[,7],lag=40,plot=FALSE),main="Original Electricity Demand"),
  autoplot(Acf(demand_deseason,lag=40,plot=FALSE),main="Deseason Electricity Demand"),
  autoplot(Pacf(demand_deseason,lag=40,plot=FALSE),main="Deseason Electricity Demand"),
  nrow=3
)
```
...
```{r Stationary test}
#Mann-Kendall
summary(MannKendall(ts_all)) #reject the null hypothesis, supporting the presence of a trend
...
#training 
msts_training<-msts(demand_deseason, seasonal.periods=c(7,365),
                   start=c(2015,07,01),end=c(2020,12,31))

#testing
msts_testing<-msts(demand_deseason, seasonal.periods=c(7,365),
...
#assuming a forecasting horizon of ten years
...
NN_forecast_all_2<-forecast(NN_fit_all_2,
                          xreg=fourier(msts_all[,7],K=c(2,12),h=365*10), 
                          h=365*10)
